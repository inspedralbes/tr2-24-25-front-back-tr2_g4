...
function createTables() {
    return new Promise((resolve, reject) => {
        connection.changeUser({ database: dataBaseName }, (err) => {
            ...
            const executeCreateTable = (index) => {
                if (index >= sqlStatementsCreate.length) {
                    resolve();
                    return;
                }
                connection.query(sqlStatementsCreate[index], (err) => {
                    if (err) {
                    ...
                    executeCreateTable(index + 1);
                });
            };

            executeCreateTable(0);
        });
    });
}
...
function insertDataTables() {
    return new Promise((resolve, reject) => {
        ...
        connection.changeUser({ database: dataBaseName }, (err) => {
            ...
            const executeInsertData = (index) => {
                if (index >= insertStatements.length) {
                    resolve();
                    return;
                }
                connection.query(insertStatements[index], (err) => {
                    if (err) {
                        console.error('Error inserting data:', err);
                        reject(err);
                        return;
                    }
                    console.log('Insert creado correctamente');
                    executeInsertData(index + 1);
                });
            };
